...
if __name__ == '__main__':
    ...
    with np.load('../1_MNIST/mnist.npz') as data:
    ...
    #carlos.load_hyperparam("../1_MNIST/hyper_mnist_sgd_batches.npz")

    es,ps = [],[]

    try:
        #brain.stochastic_gradient_descent(data[:-50], lbls[:-50], 1000000) stochastic_gradient_descent() es viejo, ahora esta SGD() mejorado

        es,ps = carlos.SGD(training_images, training_labels, mini_batch_size=50, epochs=5, test_imgs = test_img, test_lbls = test_labels)
        plt.plot(es,ps)
        plt.show()
        #pass
